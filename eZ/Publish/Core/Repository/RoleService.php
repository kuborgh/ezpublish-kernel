...
class RoleService implements RoleServiceInterface
{
    ...
    public function createRoleDraft(APIRoleCreateStruct $roleCreateStruct)
    {
        ...
        try {
            ...
            /* Throw exception if:
             * - Draft role ID is set, i.e. we are editing a role that has been published before, AND
    ...
    public function updateRoleDraft(APIRoleDraft $roleDraft, RoleUpdateStruct $roleUpdateStruct)
    {
        ...
        if ($roleUpdateStruct->identifier !== null) {
            try {
                $existingSPIRoleDraft = $this->userHandler->loadRoleDraftByIdentifier($roleUpdateStruct->identifier);
                if ($existingSPIRoleDraft instanceof SPIRole) {
                    // Throw exception, so platformui et al can do conflict management. Follow-up: EZP-24719
                    throw new InvalidArgumentException(
                        '$roleUpdateStruct',
                        "Cannot update draft for role '{$roleUpdateStruct->identifier}' because another draft exists"
                    );
                }
                ...
                */
                ...
                if ($existingRole instanceof APIRole && $existingRole->id != $roleUpdateStruct->id) {
                    throw new InvalidArgumentException(
                        '$roleUpdateStruct',
    ...
    public function addPolicyByRoleDraft(APIRoleDraft $roleDraft, APIPolicyCreateStruct $policyCreateStruct)
    {
        ...
        $loadedRoleDraft = $this->loadRole($roleDraft->id);
